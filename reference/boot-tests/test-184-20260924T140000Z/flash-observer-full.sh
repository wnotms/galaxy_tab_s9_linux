#!/usr/bin/env bash
# test-184: verified flash of the full-observer command line (vendor_boot only).
#
#   backup -> SHA256 -> flash -> readback -> SHA256 verify
#
# Only the command line moves: the same profile with the mirror and the DPU
# flight recorder switched on, for the observer-effect A/B.  boot.img is
# byte-identical to the flashed one (2e8a693f) and is left alone.
#
#   vendor_boot.img 123f35f2... -> f1f4ccf7...   (+ gts9_kmsg_mirror=1 gts9_dpu_flight=1)
#
...
BUNDLE=$REPO/out/boot-bundle-observer-full
LOG=$D/flash-observer-full-write-readback.txt
...
say "=== test-184 full-observer command line flash ==="
...
if [ "$(adbstate)" != "recovery" ]; then
	...
	powershell.exe -NoProfile -ExecutionPolicy Bypass -File "$REPO/scripts/console-run.ps1" \
		-Out 'C:\gts9-work\wd\twrp-trigger-obsfull.log' \
		-Commands 'echo TRIGGER-OBSFULL; sync; gts9-debian-to-recovery --yes' \
...
flash_one vendor_boot 123f35f29f27f7ddafe57f11015f9f0ba2da72ed2720e7567f38c0453170ffff "$BUNDLE/vendor_boot.img" || exit 1
