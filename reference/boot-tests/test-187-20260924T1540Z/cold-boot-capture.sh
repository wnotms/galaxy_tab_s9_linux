...
timeout 400 "$CR" \
	...
	-Commands 'R=/tmp/cold-post.txt; BID=$(cut -c1-8 /proc/sys/kernel/random/boot_id); D=$(ls -1d /var/log/gts9-boot-evidence/*-$BID/ 2>/dev/null | head -1); { echo "EVIDDIR=$D"; cat "$D/verdict.txt" 2>/dev/null; echo "UP=$(cut -d" " -f1 /proc/uptime)"; } > $R 2>&1; cat $R' \
