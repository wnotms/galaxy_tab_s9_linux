...
for i in $(seq 1 "$ROUNDS"); do
	...
	# A shell that answers READY and then swallows the command is the documented
	# ttyGS0 getty defect, and it cost round 2 of the RPMh run an `unattributed`
	# verdict: the probe reported `shell answered after 18 poll(s)` and then a
	# bare echo of its own command with no result.  Restarting the getty fixes it
	# (round 26), so retry once before letting a round go unattributed.
	if ! grep -aq 'RECV  boot_id=' "$DIR/probe-$i-raw.txt" 2>/dev/null; then
		# Keep the failed attempt: "the probe got nothing" and "the probe was never
		# run" look identical if the file is overwritten, and this round is about
		# exactly that distinction.
		cp "$DIR/probe-$i-raw.txt" "$DIR/probe-$i-attempt1-raw.txt" 2>/dev/null || true
		say "  probe got no result; restarting the console getty and retrying once" | tee -a "$OUT"
		timeout 120 "$CR" -Port "$SHELL_PORT" -Out "$WINDIR\\getty-$i.log" \
			-WaitReadySeconds 60 -ReadSeconds 8 \
			-Commands 'systemctl restart gts9-acm-getty.service' >/dev/null 2>&1 || true
		sleep 8
		# A fresh command file, so the retry appends rather than silently replacing
		# the evidence of the first attempt.
		run_probe "$i-retry" "$DIR/probe-$i-retry-raw.txt" "$WINDIR\\probe-$i-retry.log"
		if grep -aq 'RECV  boot_id=' "$DIR/probe-$i-retry-raw.txt" 2>/dev/null; then
			say "  the retry produced a result; using it" | tee -a "$OUT"
			cp "$DIR/probe-$i-retry-raw.txt" "$DIR/probe-$i-raw.txt"
		else
