...
| # | fact | evidence |
|---|---|---|
| 15 | **CORRECTED in round 27.** The GPU-skip parameter is **`msm.skip_gpu=1`**. `adreno_device.c` declares `static bool skip_gpu;` with `MODULE_PARM_DESC(no_gpu, ...)` and `module_param(skip_gpu, bool, 0400)`: the *description label* says `no_gpu`, the *registered parameter* is `skip_gpu`. `MODULE_PARM_DESC()` attaches a string and registers nothing, so `msm.no_gpu=1` does not exist and is silently ignored. The earlier reading of this row took the `parm` line of `modules.builtin.modinfo` as the name; the authoritative line is `parmtype`. | **device ground truth**: `/sys/module/msm/parameters/skip_gpu` exists, `/sys/module/msm/parameters/no_gpu` does not. Plus `msm.parmtype=skip_gpu:bool` in `modules.builtin.modinfo`, and the project's own working `msm.separate_gpu_kms=1` (`separate_gpu_kms = Y` on the tablet) which is the `parmtype` half of the identical `separate_gpu_drm`/`separate_gpu_kms` mismatch in `msm_drv.c` |
...
| 17 | `msm.skip_gpu=1` does **more** than disable the GPU: `skip_gpu` makes `adreno_has_gpu()` return `false` and makes `adreno_register()` return before `platform_driver_register()`. No adreno driver is ever registered, so `3d00000.gpu` never probes. `adreno_register()` is called from `msm_drm_register()` beside the display registrations and independently of them, so DPU/DSI/panel console is untouched — which is what makes profile C valid. | `adreno_device.c:187-194, 419-425`; `msm_drv.c:1135` (`adreno_register()` after `msm_dpu_register()`/`msm_dsi_register()`) |
...
| 19 | **CORRECTED in round 27.** `CONFIG_QCOM_AOSS_QMP=y` is set now, and it is not enough on its own — `CONFIG_QCOM_IPCC=y` is required too, or the IPCC node stays disabled where the RSC mailbox lives. Measured on the tablet: `c300000.power-management` is bound to driver `qcom_aoss_qmp`, and `Unable to send ACD state to AOSS` is **0** on the current boot (it was the reason the GPU could not bind before). | `out/kernel-gts9wifi/config:6873` → `CONFIG_QCOM_AOSS_QMP=y`; `kernel/config/gts9wifi-mainline.fragment:63-73`; live probe: `aoss_driver=qcom_aoss_qmp`, `acd_err=0` |
